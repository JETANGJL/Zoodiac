...
void StateHappy::Update(Backend::ECS::Component::PetComponent* _petComponent)
{
    ...
    StateUtils::FollowPlayerState(movement.m_Direction, transform.m_Position, *DI_GET_PLAYER_POS());
    if (!StateUtils::IsPetNear(transform.m_Position, *DI_GET_PLAYER_POS()))
		_petComponent->m_NextState = STATE_LIBRARY.GetResource(STATE_IDLE);
}
